# Install ValheimVRM 1.8.6 (Celeste-twinkle fork)
...
3. Extract `ValheimVRM-1.8.6.zip` directly into the folder containing `valheim.exe`.
...
disabled. Optional settings can override these values.
...
selection, install `ValheimVRM-Server-1.8.6.zip` on a BepInEx 5 server and give each
...
- Check `BepInEx/LogOutput.log` for plugin version **1.8.6**, import errors or unsupported shaders.
