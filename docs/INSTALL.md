# Install ValheimVRM 1.8.1 (Celeste-twinkle fork)
...
3. Extract `ValheimVRM-1.8.1.zip` directly into the folder containing `valheim.exe`.
...
selection, install `ValheimVRM-Server-1.8.1.zip` on a BepInEx 5 server and give each
...
- Check `BepInEx/LogOutput.log` for plugin version **1.8.1**, import errors or unsupported shaders.
