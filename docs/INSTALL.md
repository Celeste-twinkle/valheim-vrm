# Install ValheimVRM 1.8.17 (Celeste-twinkle fork)
...
3. Extract `ValheimVRM-1.8.17.zip` directly into the folder containing `valheim.exe`.
...
selection, install `ValheimVRM-Server-1.8.17.zip` on a BepInEx 5 server and give each
...
- Check `BepInEx/LogOutput.log` for plugin version **1.8.17**, import errors or unsupported shaders.
