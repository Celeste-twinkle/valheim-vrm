...
客户端和服务器推荐使用 1.8.4；同步协议仍兼容 1.8.0 客户端。推荐为参与外观同步的玩家统一分发
...
1. 服务器先安装兼容的 BepInEx 5，再将 `ValheimVRM-Server-1.8.4.zip`
   解压到服务器程序所在目录。目标为
   `BepInEx/plugins/ValheimVRM.Server/ValheimVRM.Server.dll`。
   服务器只需该同步插件，不需要客户端的着色器、UniVRM DLL 或角色模型。
2. 参与外观同步的玩家安装完整客户端 `ValheimVRM-1.8.4.zip` 和相同的模型文件夹。
...
Install the full 1.8.4 client on participating players. Their top-level
...
`ValheimVRM-Server-1.8.4.zip`, under `BepInEx/plugins/ValheimVRM.Server/`.
