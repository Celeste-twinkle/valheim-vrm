...
5. 进入世界后按 **F8** 打开人物外观面板，点击模型切换。列表可以滚动。
   插件启动时自动扫描一次；之后只有“刷新列表”会扫描并发现新增或删除的文件；F8 或 Esc 关闭面板。
...
F8 **“模型部件”**列出当前 VRM 的全部 Renderer，包括非活动层级中的部件。
逐项开关与“全部显示／模型默认／全部隐藏”按模型写入
`BepInEx/config/ValheimVRM/avatar_parts.json`。显示非活动配件时会激活所需层级；
恢复默认会从导出的 GameObject／Renderer 状态重新开始，因此刷新或重生不会重复叠加。
合并网格仍是一项；替换模型后不存在的稳定部件标识会被忽略；GPU 毛绒跟随源网格。

“同步我的模型配件设置”控制发送，“同步其他玩家的配件设置”控制接收；关闭任一开关
都不删除自己的设置文件。发送端与观察端都需为 2.0 才能显示同步后的配件状态；
1.8.14～1.8.20 的校准服务端可不修改地中转这些数据。
...
- `avatar_calibration.json`：保存动作或装备校准时生成。
- `avatar_parts.json`：修改逐项／全部部件显示状态时生成。
...
解压 `ValheimVRM-Server-2.0.2.zip`，并让各客户端安装相同模型文件，详见
...
- 查看 `BepInEx/LogOutput.log` 中的插件版本 **2.0.2**、模型导入错误和着色器错误。
