# ValheimVRM 1.8.11 安装说明（Celeste-twinkle 分支）
...
3. 将 `ValheimVRM-1.8.11.zip` 直接解压到 `valheim.exe` 所在目录，合并
...
启用 MToon、关闭旧版角色淡出；可选配置仍可覆盖这些参数，但缩放受 2 米最低高度限制。
1.8.8 起，加载时按站立可见网格高度（含头发和头饰）等比放大，实际倍率为
`max(ModelScale, 2.0 / 原始高度)`。1.2 米模型变为 2 米，已足够高或手动放大更多的保持。
...
解压 `ValheimVRM-Server-1.8.11.zip`，并让各客户端安装相同模型文件，详见
...
- 查看 `BepInEx/LogOutput.log` 中的插件版本 **1.8.11**、模型导入错误和着色器错误。
...
1.8.11 加载时根据人形参考骨架和鞋底数据一次性标定站立／走跑偏移，保留原版髋部动画，不再在走跑时实时追踪最低脚底点。坐姿仍单独处理；部分姿势可能有少量脚底穿入或离地。
