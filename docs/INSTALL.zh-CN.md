...
缺少文件使用默认值。服务端、发送端和观察端均为 1.8.14+ 时，所有校准滑块、站姿／坐姿偏移、物理权重和身高按玩家同步。接收值只修改该玩家的显示实例，不写入观察者配置；同模型玩家互不影响。微调松开后同步并原位应用；模型／身高变化从原始参考重新标定，不累积偏移。F8 显示完整校准同步是否启用。
...
1.8.14+ 完整校准同步启用后，这些微调也按玩家同步；既有 `ModelOffsetY` 全局偏移仍会叠加。
