...
毛绒接受客户端场景光照与阴影选项，继承已限制的衣料底色。自 **1.8.18** 起，毛绒同步源材质的法线贴图／强度、切线方向、明暗贴图与参数、自发光、MatCap 和边缘光；不凭空增加发光。VRM 0.x 使用原生旧版 MToon 的明暗分界、阴影／明暗等级贴图、光色衰减和基础光照上限，VRM 1.0 使用 MToon 1.0 公式，避免同一块衣料与毛绒采用不同受光规则。不投射每条纤维的细阴影，基础衣服仍投影。
...
**与 lilToon 的视觉差异：** 扩展只携带上述毛绒几何／遮罩参数，光照以导入后的 VRM MToon 材质为准。它没有携带 lilToon 的全部分层阴影、反射、专用亮度限制或材质功能，所以不能承诺与 Unity 中原 lilToon 逐像素相同。底色、法线或明暗色不同的配饰仍会有真实明暗差异；关闭“模型泛光”也不会降低太阳／火把照亮的材质本身。需要当前光照一致性修复时，所有观察者使用 **1.8.18 或更新版客户端**，无需重导出已有毛绒 VRM。
