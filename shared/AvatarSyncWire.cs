...
namespace ValheimVRM.Sync
{
    public static class AvatarSyncWire
    {
        ...
        public static bool ReadSelection(ZPackage p, out bool enabled, out string model, out string hash, out long sequence, out float height, out bool withHeight)
        {
            ...
            height = AvatarHeightRules.Default; withHeight = false;
            try
            {
                ...
                if (p.GetPos() != p.Size())
                {
                    ...
                    if (remaining != sizeof(long) && remaining != sizeof(long) + sizeof(float)) return false;
                    sequence = p.ReadLong();
                    if (sequence <= 0) return false;
                    if (remaining == sizeof(long) + sizeof(float))
                    {
        ...
        public static ZPackage Snapshot(long revision, AvatarSelection[] states, bool withHeight = false)
        {
            var p = new ZPackage(); p.Write(withHeight ? HeightSnapshotVersion : AvatarSyncRules.Version); p.Write(revision); p.Write(states.Length);
        ...
        {
            revision = -1; states = null; withHeight = false;
            try
            {
                if (p == null || p.Size() > 256 * 1024) return false;
                int version = p.ReadInt();
                if (version != AvatarSyncRules.Version && version != HeightSnapshotVersion) return false;
                withHeight = version == HeightSnapshotVersion;
