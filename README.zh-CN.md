...
**当前发布版：1.8.14** · 验证环境：Valheim **1.0.12**、Windows x64、Unity 6000.0.75f1、BepInEx **5.4.23.3**、D3D11。
...
- 自动标定游戏动画姿态；F8 按动画名展开三轴微调，左手／右手／双手道具分别调整大小和位置。
...
| 使用场景 | 安装内容 |
| --- | --- |
| 玩家客户端，含单人、本地外观和联机外观 | BepInEx 5 + `ValheimVRM-1.8.14.zip` + 自备 `.vrm`。 |
| 专用服务器，需要同步玩家外观 | BepInEx 5 + `ValheimVRM-Server-1.8.14.zip`；无需模型或客户端依赖。 |
| 通过游戏“启动服务器”的房主，需要外观同步 | 在房主游戏目录安装客户端包和服务器包；其他玩家按客户端方式安装。 |
| 仅下载源码 | `ValheimVRM-1.8.14-source.zip` 用于开发，不能代替编译好的插件包。 |
...
另外分发的**本地 Windows x64 完整包** `09_ValheimVRM_1.8.14_完整插件.zip` 和 `10_ValheimVRM_Server_1.8.14.zip` 均已包含 BepInEx **5.4.23.3**，首次安装无需再下载前置。安装前退出游戏／停止服务器。服务器已有兼容 BepInEx 时，只复制本地服务端 ZIP 中的 `BepInEx/plugins/ValheimVRM.Server`，保留原加载器和配置。
...
5. 在 `BepInEx/LogOutput.log` 中确认加载的是 **ValheimVRM 1.8.14**。
...
| `BepInEx/config/ValheimVRM/avatar_calibration.json` | 按模型保存的动作三轴偏移、左／右／双手道具缩放与偏移。 | 保存 F8 校准滑块时生成。 |
...
微调按模型写入 `BepInEx/config/ValheimVRM/settings_模型名.txt` 的 `StandingHeightOffset`、`SittingHeightOffset`（单位米）。缺少文件时自动创建，保留其他参数和注释。服务端、发送端及观察端均为 1.8.14+ 时，这些微调按玩家同步；原有 `ModelOffsetY` 仍会叠加全局偏移。滑块用于特殊鞋底、服装形状的补充校准；默认自动定位仍然生效，也不会为所有椅子自动增加屈膝 IK。
...
无服务端插件或关闭同步时，身高调整只在本机生效。旧版服务器／客户端仍可同步模型，但不支持自定义远程身高；新版观察者将旧版发送方按默认 2 米显示。F8 会提示是否支持身高同步。站姿、坐姿偏移仍按模型保存个人偏好，并在 1.8.14+ 中纳入完整的玩家校准同步。
...
在专用服务器上安装 BepInEx 5，停止服务器，将完整 `ValheimVRM-Server-1.8.14.zip` 解压到服务器根目录，再按原启动方式启动：
...
参与同步的玩家安装完整客户端，并准备需要显示的同名、同内容 `.vrm`。进入世界后按 F8，保持同步勾选，确认“服务器同步已连接”。推荐服务器和参与同步的客户端统一为 **1.8.14**。完整校准同步要求服务端、发送端和观察端均为 1.8.14+，F8 显示是否启用；旧版继续保留其支持的模型／身高同步。
...
整个模型文件夹无需完全一致：只有要显示的模型需要**相同文件名（含大小写）与 SHA-256 内容指纹**。服务器中转名称、指纹、身高和校准数值，不传输 VRM。补齐未导入的文件后刷新列表重试；替换已缓存的同名文件后重启客户端。1.8.14+ 会同步各玩家的校准参数和物理权重，接收方保留自己的配置文件；渲染开关仍是观察者的画面偏好。
...
| 退出世界后越来越卡、报显存分配错误 | 更新完整 **1.8.14** 并排除重复 DLL。该版已修复主菜单重入时重复补丁造成的显存泄漏。 |
...
联机验证覆盖实际 ZRpc 序列化、生产服务端处理器和两名角色的独立绑定；尚未完成真实 Steam／PlayFab 专用服务器公网验收。Linux、macOS、Vulkan 和所有其他 Mod 组合未得到全面验证。1.8.14 的动作／持物校准验证见[验证记录](https://github.com/Celeste-twinkle/valheim-vrm/blob/codex/public-release/docs/release-1.8.14-validation.md)。1.8.12 的贴地与姿态回归见[验证记录](https://github.com/Celeste-twinkle/valheim-vrm/blob/codex/public-release/docs/release-1.8.12-validation.md)。旧版生命周期测试见[1.8.7 验证记录](https://github.com/Celeste-twinkle/valheim-vrm/blob/codex/public-release/docs/release-1.8.7-validation.md)。
...
输出为 `release/ValheimVRM-1.8.14.zip` 和 `release/ValheimVRM-Server-1.8.14.zip`。客户端构建会清理 release 目录，应先构建客户端，再打服务器包。默认不安装到游戏；只有显式传入 `-p:InstallToGame=true` 才安装。必须完整构建以嵌入渲染资源，不能用 `-t:Compile` 输出代替发布包。
