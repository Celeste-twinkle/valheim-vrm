...
上述微调默认位移 0、缩放 100%，拖动即时预览、松开或关闭菜单保存；每项可复位。配置位于 `BepInEx/config/ValheimVRM/avatar_calibration.json`，无需在模型文件夹添加文件。它们与现有站姿／坐姿高度微调共同生效，**只按模型保存在本机，不额外同步**；远端模型仍按各玩家已同步的身高分别自动标定和缩放道具。VRM 0.x／1.0 共用此流程。不同体型可能保留少量穿插或接触间隙，固定标定不等于逐帧手脚 IK。
...
参与同步的玩家安装完整客户端，并准备需要显示的同名、同内容 `.vrm`。进入世界后按 F8，保持同步勾选，确认“服务器同步已连接”。推荐服务器和参与同步的客户端统一为 **1.8.14**。
