...
static class ServerRelayProbe
{
    public static void Run(string[] models,string[] hashes)
    {
        ...
        try
        {
            ...
            for(int i=0;i<30;i++)clients[0].Invoke(AvatarSyncWire.Select,AvatarSyncWire.Selection(true,models[i%2],hashes[i%2]));
            pump();
            ...
            clients[0].Invoke(AvatarSyncWire.Select,AvatarSyncWire.Selection(false,"",""));pump();
    ...
    {
        public RelaySocket Other;readonly Queue<ZPackage> packets=new Queue<ZPackage>();bool open=true;
