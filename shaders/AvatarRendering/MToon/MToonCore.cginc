...
// This variant is used only when scene lighting or shadow reception is disabled.
// Bypass shadow sampling while retaining point/spot distance and cookie falloff.
#undef UNITY_SHADOW_ATTENUATION
#define UNITY_SHADOW_ATTENUATION(a, worldPos) 1.0
float _AvatarSceneLighting;
...
float4 frag_forward(v2f i) : SV_TARGET
{
    ...
    // Match the MToon10 scene-lighting switch: base + authored emission once,
    // with zero contribution from additional lights. Retain alpha, UVs and fog.
    if (_AvatarSceneLighting < 0.5)
    {
        #ifdef MTOON_FORWARD_ADD
        return half4(0, 0, 0, alpha);
        #else
        half3 flatColor = _Color.rgb * mainTex.rgb + tex2D(_EmissionMap, mainUv).rgb * _EmissionColor.rgb;
        flatColor = lerp(flatColor, _OutlineColor.rgb, i.isOutline);
        half4 flatResult = half4(flatColor, alpha);
        UNITY_APPLY_FOG(i.fogCoord, flatResult);
        return flatResult;
        #endif
    }
