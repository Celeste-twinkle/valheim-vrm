Shader "ValheimVRM/Fur"
{
    Properties
    {
        ...
        // Lighting
        _AvatarSceneLighting ("Scene lighting", Float) = 1
        ...
        _FurLength ("Fur length in model meters", Float) = 0
        _FurDensity ("Fins per triangle", Float) = 2
        _FurDirection ("Tangent space direction", Vector) = (0,0,1,0)
        _FurRandomness ("Direction variation", Range(0,1)) = 0.5
        _FurRootOffset ("Fur root coverage", Range(-1,0)) = -1
        _FurLengthMask ("Length mask (R)", 2D) = "white" {}
        _FurNoise ("Strand noise (R)", 2D) = "white" {}
        _FurMask ("Coverage mask (R)", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            HLSLPROGRAM
            #pragma target 4.0
            #pragma multi_compile_local __ AVATAR_FUR_ON
        ...
        {
            Name "BLOOM_MASK"
            Tags { "LightMode" = "ValheimVRMBloomMask" }
            Cull Off
            ZWrite Off
            ZTest LEqual
            Blend One One
