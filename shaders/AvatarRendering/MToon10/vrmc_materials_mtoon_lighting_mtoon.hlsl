...
half4 GetMToonLighting(const UnityLighting unityLight, const MToonInput input)
{
    ...
    {
        if (!MToon_IsForwardBasePass()) return half4(0, 0, 0, input.alpha);
        half3 color = input.litColor + GetMToonLighting_Emissive(input);
        if (MToon_IsOutlinePass()) color = _OutlineColor.rgb;
